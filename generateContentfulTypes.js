...
const formatType = (field, prefix = '', isArray = false) => {
  ...
  case 'Object': // JSON object
    return 'any'
  case 'RichText':
    return '{ content: TopLevelBlock[], data: NodeData, nodeType: BLOCKS.DOCUMENT }' // Use type directly from contentful.js when available
  case 'Link':
...
const generateContentfulTypes = (outputFilePath = './contentfulTypes.d.ts', host = 'cdn.contentful.com', environment = 'master', prefix = '', ignoredFields) => {
...
program
  ...
  .action((options) => generateContentfulTypes(options.output, options.host, options.environment, options.prefix, options.ignore))
  .parse();
